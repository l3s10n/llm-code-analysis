...
通过对广泛使用的开源项目（star 数从 1.4k 到 38.2k 不等）进行测试，VulSolver 展现了其在大型实际代码库中进行漏洞分析的有效性。截至 2026 年 4 月 3 日，VulSolver 已发现 38 个零日漏洞，其中大部分是在项目近期架构优化后发现的。它们的 CVE 编号正在申请中，后续会补充。部分已获得的 CVE 编号如下：
