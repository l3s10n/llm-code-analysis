...
VulSolver 目前支持路径穿越、命令执行、代码执行、SQL 注入和 SSRF 漏洞的检测,并正在持续扩展以支持更多漏洞类型。

通过对广泛使用的开源项目（star 数从 1.4k 到 38.2k 不等）进行测试，VulSolver 展现了其在大型实际代码库中进行漏洞分析的有效性。截至 2026 年 4 月 16 日，VulSolver 已发现 40+ 个零日漏洞，其中大部分是在项目近期架构优化后发现的。它们的 CVE 编号正在申请中，后续会补充。部分已获得的 CVE 编号如下：
...
注：所有漏洞均由我们的基于 LLM 的项目 100% 独立发现，无任何人工辅助。这些漏洞在发现时均为零日状态，但由于与其他研究者同期重复提交，部分 CVE 编号被分配给了其他发现者。
